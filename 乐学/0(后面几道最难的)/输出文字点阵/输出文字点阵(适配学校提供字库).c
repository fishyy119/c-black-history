...
#include <stdlib.h>
/******************************************************
*  学校提供的字库缺少非汉字字符，所有汉字的区码       *
*  减15才可得到在此字库中对应的汉字(区码1 ~ 15        *
*  是非汉字字符，学校提供字库中没有)                  *
*******************************************************
*  但本程序无法用于提交，本程序为了显示效果在         *
*  各点间插入了空格，无法通过乐学检测。               * 
*******************************************************/
//TODO 免去繁琐的输入 
//可选字库如下
...
int main(void){ 
	...
	fp = fopen(fileName, "r");
	fseek(fp, ((position / 100 - 1 - 15) * 94 + position % 100 - 1) * (n * (n / 8 + (n % 8 != 0))), SEEK_SET); // 若n不是8的倍数, 则(n % 8 != 0)的值为一 
	for(i = 0; i < n; i++){ // 汉字行数
		count = 0;
